...
**     Component   : USB_LDD
**     Version     : Component 01.305, Driver 01.08, CPU db: 3.00.000
**     Compiler    : GNU C Compiler
**     Date/Time   : 2012-10-22, 10:05, # CodeGen: 14
**     Abstract    :
...
#define USB_LDD_AVAILABLE_EVENTS_MASK ((LDD_TEventMask)( \
          ...
          LDD_USB_ON_DEVICE_SOF | \
          LDD_USB_ON_DEVICE_ERROR | \
          LDD_USB_ON_DEVICE_1_MS_TIMER))
...
typedef struct USB_LDD_TDeviceData_Struct {
  ...
  LDD_RTOS_TISRVectorSettings SavedISRSettings_USB_ISR; /* {MQXLite RTOS Adapter} Saved settings of allocated interrupt vector */
...
static void USB_LDD_DeviceDisableAllEp(USB_LDD_TDeviceData *DevDataPtr)
{
  ...
  if (DevDataPtr->EpData[x].pCurrentTD != NULL)
  if (DevDataPtr->EpData[x].pCurrentTD->UserTDPtr != NULL)
  DevDataPtr->EpData[x].pCurrentTD->UserTDPtr->CallbackFnPtr = 0;
  }
...
LDD_TDeviceData * USB_LDD_Init(LDD_TUserData *UserDataPtr)
{
  ...
  /* {MQXLite RTOS Adapter} Driver memory allocation: Fill the allocated memory by zero value */
  ...
  DevDataPtr->SavedISRSettings_USB_ISR.isrData = _int_get_isr_data(LDD_ivIndex_INT_USB0);
  DevDataPtr->SavedISRSettings_USB_ISR.isrFunction = _int_install_isr(LDD_ivIndex_INT_USB0, USB_LDD_USB_Interrupt,  DevDataPtr);
  /* Interrupt vector(s) priority setting */
  /* NVIC_IPR6: PRI_24=0x80 */
  NVIC_IPR6 = (uint32_t)((NVIC_IPR6 & (uint32_t)~0x7FUL) | (uint32_t)0x80UL);
  /* NVIC_ISER: SETENA|=0x01000000 */
  NVIC_ISER |= (uint32_t)0x01000000UL;                       
  /* Clock setting */
...
void USB_LDD_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)_int_install_isr(LDD_ivIndex_INT_USB0, DevDataPtr->SavedISRSettings_USB_ISR.isrFunction, DevDataPtr->SavedISRSettings_USB_ISR.isrData);
...
LDD_TError USB_LDD_SetEventMask(LDD_TDeviceData *DeviceDataPtr, LDD_TEventMask EventMask)
{
  ...
  if (((uint32_t)EventMask & LDD_USB_ON_DEVICE_1MS_TIMER) != 0U) {
    USB_PDD_Enable1msInterrupt(USB0_BASE_PTR);
  }
  USB_LDD_UsbUnlock(DevDataPtr);
  return ERR_OK;
}
...
static void USB_LDD_DeviceIsr(USB_LDD_TDeviceData *DevDataPtr, uint32_t InSignalsState)
{
  ...
  if (DevDataPtr->DeviceState >= LDD_USB_DEVICE_VBUS_OFF) { /* Device enabled and VBUS = ON? */
    if ((InSignalsState & USB_LDD_1MS_TIMEOUT) != 0x00U) {  /* 1ms timer interrupt? */
      ...
      if (DevDataPtr->EventMask & LDD_USB_ON_DEVICE_1_MS_TIMER) {
    ...
    if ((IntStatus & USB_PDD_SOF_TOK_INT) != 0x00U) {       /* SOF received? */
      ...
      if (DevDataPtr->EventMask & LDD_USB_ON_DEVICE_SOF) {
    ...
    if ((IntStatus & USB_PDD_TOK_DNE_INT) != 0x00U) {
      ...
      /* USB Reset signal detected interrupt handler */
        ...
        if (USB_PDD_GetDpPullUpSignalState(USB0_BASE_PTR)) { /* VBUS on? */
          if ((DevDataPtr->EventMask & LDD_USB_ON_DEVICE_SOF) != 0x00U) {
...
void USB_LDD_USB_Interrupt(LDD_RTOS_TISRParameter _isrParameter)
{
  ...
  USB_LDD_TDeviceDataPtr DevDataPtr = (USB_LDD_TDeviceDataPtr)_isrParameter;
  ...
  if ((OtgIntStatus & USB_PDD_1_MSEC_INT) != 0U) {         /* 1ms timer interrupt? */
    ...
    if (DevDataPtr->msCounter > 0x00) {                    /* Otg timing in progress? */
      ...
      if (DevDataPtr->msCounter == 0x00 ) {                /* Timing done? */
        ...
        if (((uint32_t)(DevDataPtr->EventMask) & LDD_USB_ON_DEVICE_1MS_TIMER) == 0x00U) { /* No, user timing request? */
