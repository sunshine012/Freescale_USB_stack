...
int __read_console(__file_handle handle, unsigned char* buffer, size_t * count)
{
    ...
    for (i = 0; i < *count; i++) {
        ...
        while ((UART_PDD_ReadInterruptStatusReg(UART3_BASE_PTR) & UART_S1_RDRF_MASK) == 0) {};

        /* Save a character of the UARTx device into the receive buffer */
        buffer[i] = (unsigned char)UART_PDD_GetChar8(UART3_BASE_PTR);
    }
...
{
    while ((UART_PDD_ReadInterruptStatusReg(UART3_BASE_PTR) & UART_S1_TDRE_MASK) == 0) {};
    UART_PDD_PutChar8(UART3_BASE_PTR, Char);
}
...
uint32_t CsIO1_ReadLine(char *BufferPtr, uint32_t BufferSize)
{
    ...
    if (((uint32_t)BufferPtr | (uint32_t)BufferSize) == 0u) { /* BufferPtr == NULL or BufferSize == 0?*/
        ...
        if ((UART_PDD_ReadInterruptStatusReg(UART3_BASE_PTR) & UART_S1_RDRF_MASK) != 0) { /* Character recieved? */
            c = (uint8_t)UART_PDD_GetChar8(UART3_BASE_PTR); /* Read char from HW */
