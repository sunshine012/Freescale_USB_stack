...
LDD_TDeviceData* OTG_MAX3353_INT_PIN_Init(LDD_TUserData *UserDataPtr)
{
  ...
  DeviceDataPrv->SavedISRSettings_PortInterrupt.isrData = _int_get_isr_data(LDD_ivIndex_INT_PORTB);
  DeviceDataPrv->SavedISRSettings_PortInterrupt.isrFunction = _int_install_isr(LDD_ivIndex_INT_PORTB, OTG_MAX3353_INT_PIN_Interrupt, DeviceDataPrv);
  /* GPIOB_PDDR: PDD&=~0x80 */
  GPIOB_PDDR &= (uint32_t)~(uint32_t)(GPIO_PDDR_PDD(0x80));                                                   
  /* Initialization of Port Control registers */
  /* PORTB_PCR7: ISF=0,LK=0,MUX=1 */
...
void OTG_MAX3353_INT_PIN_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  /* PORTB_PCR7: IRQC=0 */
  PORTB_PCR7 &= (uint32_t)~(uint32_t)(PORT_PCR_IRQC(0x0F));                                                   
  /* GPIOB_PDDR: PDD&=~0x80 */
  GPIOB_PDDR &= (uint32_t)~(uint32_t)(GPIO_PDDR_PDD(0x80));                                                   
  ...
  (void)_int_install_isr(LDD_ivIndex_INT_PORTB, ((OTG_MAX3353_INT_PIN_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings_PortInterrupt.isrFunction, ((OTG_MAX3353_INT_PIN_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings_PortInterrupt.isrData);
...
LDD_TError OTG_MAX3353_INT_PIN_ConnectPin(LDD_TDeviceData *DeviceDataPtr, LDD_TPinMask PinMask)
{
  ...
  if ((PinMask & LDD_GPIO_PIN_7) != 0U) {
    clrSetReg32Bits(PORTB_PCR7, 0x01000600U, 0x0100U); /* MUX=0x01U */
  }
  return ERR_OK;
}
