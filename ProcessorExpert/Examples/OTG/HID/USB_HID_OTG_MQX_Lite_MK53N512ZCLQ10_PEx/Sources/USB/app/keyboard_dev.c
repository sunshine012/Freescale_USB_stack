...
static uint_8       g_app_request_params[2];          /* for get/set idle and
                                                      protocol requests */
_task_id 			dev_app_task_id = MQX_NULL_TASK_ID; /* device task ID */
uint_8 				g_device_app_stack[DEVICE_APP_TASK_TASK_STACK_SIZE]; /* device stack */
/*****************************************************************************
* Local Functions
*****************************************************************************/
/******************************************************************************
...
uint_32 App_PeripheralInit(void)
{
    ...
    USB_lock();
    dev_app_task_id = _task_create_at(0, DEVICE_APP_TASK_TASK, 0, g_device_app_stack, DEVICE_APP_TASK_TASK_STACK_SIZE);
...
void App_PeripheralUninit(void)
{
    ...
    if(dev_app_task_id != MQX_NULL_TASK_ID)
	{
    	_task_destroy(dev_app_task_id);  
	}
    USB_unlock();
    host_stack_active  = FALSE;
