...
LDD_TDeviceData* OTG_MAX3353_INT_PIN_Init(LDD_TUserData *UserDataPtr)
{
  ...
  DeviceDataPrv->SavedISRSettings_PortInterrupt.isrData = _int_get_isr_data(LDD_ivIndex_INT_PORTD);
  DeviceDataPrv->SavedISRSettings_PortInterrupt.isrFunction = _int_install_isr(LDD_ivIndex_INT_PORTD, OTG_MAX3353_INT_PIN_Interrupt, DeviceDataPrv);
  /* GPIOD_PDDR: PDD&=~2 */
...
void OTG_MAX3353_INT_PIN_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  /* PORTD_PCR1: IRQC=0 */
  PORTD_PCR1 &= (uint32_t)~(uint32_t)(PORT_PCR_IRQC(0x0F));                                                   
  /* GPIOD_PDDR: PDD&=~2 */
  GPIOD_PDDR &= (uint32_t)~(uint32_t)(GPIO_PDDR_PDD(0x02));                                                   
  ...
  (void)_int_install_isr(LDD_ivIndex_INT_PORTD, ((OTG_MAX3353_INT_PIN_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings_PortInterrupt.isrFunction, ((OTG_MAX3353_INT_PIN_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings_PortInterrupt.isrData);
...
LDD_TError OTG_MAX3353_INT_PIN_ConnectPin(LDD_TDeviceData *DeviceDataPtr, LDD_TPinMask PinMask)
{
  ...
  if ((PinMask & LDD_GPIO_PIN_1) != 0U) {
    clrSetReg32Bits(PORTD_PCR1, 0x01000600U, 0x0100U); /* MUX=0x01U */
  }
  return ERR_OK;
}
