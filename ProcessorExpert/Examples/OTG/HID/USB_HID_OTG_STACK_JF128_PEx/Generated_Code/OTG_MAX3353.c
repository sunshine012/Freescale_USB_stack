...
**            Pins                                         : 
**              SDA pin                                    : 
**                SDA pin                                  : PTC7/UART0_RX/I2C0_SDA/RGPIO7/SPI1_MISO/FBa_AD12
**                SDA pin signal                           : 
**              SCL pin                                    : 
**                SCL pin                                  : PTC6/UART0_TX/I2C0_SCL/RGPIO6/SPI1_MOSI/FBa_AD11
**                SCL pin signal                           : 
...
**            SCL frequency                                : 399.77 kHz
**            SDA Hold                                     : 0.75 us
**            SCL start Hold                               : 0.917 us
**            SCL stop Hold                                : 1.334 us
...
**         Enable                       - LDD_TError OTG_MAX3353_Enable(LDD_TDeviceData *DeviceDataPtr);
**         Disable                      - LDD_TError OTG_MAX3353_Disable(LDD_TDeviceData *DeviceDataPtr);
...
**         GetStats                     - LDD_I2C_TStats OTG_MAX3353_GetStats(LDD_TDeviceData *DeviceDataPtr);
...
} OTG_MAX3353_TDeviceData;

typedef OTG_MAX3353_TDeviceData *OTG_MAX3353_TDeviceDataPtr; /* Pointer to the device data structure. */

/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static OTG_MAX3353_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
{
  OTG_MAX3353_TDeviceDataPtr DeviceDataPrv = (OTG_MAX3353_TDeviceDataPtr)DeviceDataPtr;
  ...
  if ((I2C_PDD_ReadStatusReg(I2C0_BASE_PTR) & I2C_PDD_INTERRUPT_FLAG) != 0x00U) {
      ...
      if (I2C_PDD_GetTransmitMode(I2C0_BASE_PTR) == I2C_PDD_TX_DIRECTION) { /* Is device in Tx mode? */
          ...
          if ((DeviceDataPrv->SerFlag & ADDR_COMPLETE) != 0x00U) { /* If 10-bit addr has been completed */
            if (DeviceDataPrv->OutLenM != 0x00U) { /* Is any char. for transmitting? */
              ...
              if (DeviceDataPrv->InpLenM != 0x00U) { /* Is any char. for reception? */
                ...
                OTG_MAX3353_OnMasterBlockSent(DeviceDataPrv->UserData); /* Invoke OnMasterBlockSent event */
              }
            }
          } else {
        ...
        *(DeviceDataPrv->InpPtrM)++ = I2C_PDD_ReadDataReg(I2C0_BASE_PTR); /* Receive character */
        ...
        if (DeviceDataPrv->InpLenM == 0x00U) { /* Is any char. for reception? */
          ...
          OTG_MAX3353_OnMasterBlockReceived(DeviceDataPrv->UserData); /* Invoke OnMasterBlockReceived event */
        }
      }
    } else {
    ...
    if (ErrorMask != 0x00U) {          /* Is any error mask set? */
      ...
      OTG_MAX3353_OnError(DeviceDataPrv->UserData); /* If yes then invoke user event */
    }
  }
}
...
LDD_TDeviceData* OTG_MAX3353_Init(LDD_TUserData *UserDataPtr)
{
  ...
  OTG_MAX3353_TDeviceData *DeviceDataPrv;
  ...
  DeviceDataPrv->SlaveAddr = 0x58U;    /* Set variable for slave address */
  DeviceDataPrv->SendStop = LDD_I2C_SEND_STOP; /* Set variable for sending stop condition (for master mode) */
  ...
  /* SIM_SCGC1: I2C0=1 */
  SIM_SCGC1 |= (uint8_t)0x10U;                            
  /* I2C0_C1: IICEN=0,IICIE=0,MST=0,TX=0,TXAK=0,RSTA=0,WUEN=0,DMAEN=0 */
  I2C0_C1 = (uint8_t)0x00U;            /* Clear control register */
  /* I2C0_S: TCF=0,IAAS=0,BUSY=0,ARBL=0,RAM=0,SRW=0,IICIF=1,RXAK=0 */
  I2C0_S = (uint8_t)0x02U;             /* Clear interrupt flag */
  /* MXC_PTCPF1: C7=3 */
  MXC_PTCPF1 = (uint8_t)((MXC_PTCPF1 & (uint8_t)~(uint8_t)0xC0U) | (uint8_t)0x30U);
  /* MXC_PTCPF1: C6=3 */
  MXC_PTCPF1 = (uint8_t)((MXC_PTCPF1 & (uint8_t)~(uint8_t)0x0CU) | (uint8_t)0x03U);
  /* I2C0_C2: GCAEN=0,ADEXT=0,HDRS=0,SBRC=0,RMEN=0,AD=0 */
  I2C0_C2 = (uint8_t)0x00U;                          
  /* I2C0_FLT: ??=0,??=0,??=0,??=0,FLT=0 */
  I2C0_FLT = (uint8_t)0x00U;           /* Set glitch filter register */
  /* I2C0_SMB: FACK=0,ALERTEN=0,SIICAEN=0,TCKSEL=0,SLTF=1,SHTF1=0,SHTF2=0,SHTF2IE=0 */
  I2C0_SMB = (uint8_t)0x08U;                          
  /* I2C0_F: MULT=0,ICR=5 */
  I2C0_F = (uint8_t)0x05U;             /* Set prescaler bits */
  HWEnDi(DeviceDataPrv);               /* Enable/disable device according to status flags */
  /* Registration of the device structure */
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_OTG_MAX3353_ID,DeviceDataPrv);
...
void OTG_MAX3353_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  /* I2C0_C1: IICEN=0,IICIE=0,MST=0,TX=0,TXAK=0,RSTA=0,WUEN=0,DMAEN=0 */
  I2C0_C1 = (uint8_t)0x00U;            /* Reset I2C Control register */
  /* Restoring the interrupt vector */
  /* Unregistration of the device structure */
  PE_LDD_UnregisterDeviceStructure(PE_LDD_COMPONENT_OTG_MAX3353_ID);
  /* Deallocation of the device structure */
  /* {Default RTOS Adapter} Driver memory deallocation: Dynamic allocation is simulated, no deallocation code is generated */
  /* SIM_SCGC1: I2C0=0 */
  SIM_SCGC1 &= (uint8_t)~(uint8_t)0x10U;                           
}

/*
** ===================================================================
**     Method      :  OTG_MAX3353_Enable (component I2C_LDD)
**
**     Description :
...
** ===================================================================
*/
LDD_TError OTG_MAX3353_Enable(LDD_TDeviceData *DeviceDataPtr)
{
  OTG_MAX3353_TDeviceData *DeviceDataPrv = (OTG_MAX3353_TDeviceData *)DeviceDataPtr;
...
/*
** ===================================================================
**     Method      :  OTG_MAX3353_Disable (component I2C_LDD)
**
**     Description :
...
** ===================================================================
*/
LDD_TError OTG_MAX3353_Disable(LDD_TDeviceData *DeviceDataPtr)
{
  OTG_MAX3353_TDeviceData *DeviceDataPrv = (OTG_MAX3353_TDeviceData *)DeviceDataPtr;
...
bool OTG_MAX3353_MasterGetBlockSentStatus(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  Status = ((OTG_MAX3353_TDeviceDataPtr)DeviceDataPtr)->StatusFlag; /* Save flag for return */
  ((OTG_MAX3353_TDeviceDataPtr)DeviceDataPtr)->StatusFlag &= (uint8_t)(~(uint8_t)MASTER_BLOCK_SENT); /* Clear data block sent flag */
...
bool OTG_MAX3353_MasterGetBlockReceivedStatus(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  Status = ((OTG_MAX3353_TDeviceDataPtr)DeviceDataPtr)->StatusFlag; /* Save flag for return */
  ((OTG_MAX3353_TDeviceDataPtr)DeviceDataPtr)->StatusFlag &= (uint8_t)(~(uint8_t)MASTER_BLOCK_RECEIVED); /* Clear data block received flag */
...
/*
** ===================================================================
**     Method      :  OTG_MAX3353_GetStats (component I2C_LDD)
**
**     Description :
...
** ===================================================================
*/
LDD_I2C_TStats OTG_MAX3353_GetStats(LDD_TDeviceData *DeviceDataPtr)
{
  OTG_MAX3353_TDeviceData *DeviceDataPrv = (OTG_MAX3353_TDeviceData *)DeviceDataPtr;
