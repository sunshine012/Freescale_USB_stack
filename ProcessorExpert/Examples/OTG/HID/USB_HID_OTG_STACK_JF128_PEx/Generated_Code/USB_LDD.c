...
static USB_LDD_TDeviceDataPtr Vusb0__DEFAULT_RTOS_ISRPARAM;
...
*/
#ifdef _lint
uint16_t USB_LDD_ByteRev16(uint16_t Value);
#else
asm static inline __declspec (register_abi) uint16_t USB_LDD_ByteRev16(uint16_t Value:__D0);
asm static inline __declspec (register_abi) uint16_t USB_LDD_ByteRev16(uint16_t Value:__D0) {
  BYTEREV  D0
  SWAP     D0
}
...
*/
#ifdef _lint
uint32_t USB_LDD_ByteRev32(uint32_t Value);
#else
asm static inline __declspec (register_abi) uint32_t USB_LDD_ByteRev32(uint32_t Value:__D0);
asm static inline __declspec (register_abi) uint32_t USB_LDD_ByteRev32(uint32_t Value:__D0) {
  BYTEREV  D0
}
#endif
...
static void USB_LDD_DeviceQueueDataPacket(USB_LDD_TEpData *EpDataPtr, USB_LDD_TTD *TDPtr) {
  ...
  do {
    ...
    BDPtr->BufferPtr = USB_LDD_ByteRev32((uint32_t)TDPtr->BufferPtr); /* Set buffer address */
    BDPtr->CtrlReg = USB_LDD_ByteRev32(((uint32_t)PacketSize << 0x10U) | EpDataPtr->BDCtrlRegVal); /* Set packet size and put this entry to the SIE */
...
static void USB_LDD_HostStartTransaction(USB_LDD_TDeviceData *DevDataPtr) {
  ...
  BDPtr->BufferPtr = USB_LDD_ByteRev32((uint32_t)BufferPtr);         /* Set buffer address */
  BDPtr->CtrlReg = USB_LDD_ByteRev32(BDCtrlRegVal);                  /* Set packet properties */
...
LDD_TDeviceData * USB_LDD_Init(LDD_TUserData *UserDataPtr)
{
  ...
  Vusb0__DEFAULT_RTOS_ISRPARAM =  DevDataPtr;
  /* Interrupt vector(s) priority setting */
  /* INTC_IMRL: IMR9=0 */
  INTC_IMRL &= (uint32_t)~0x0200UL;                      
  /* Set clock source divider and multiplier */
  /* SIM_CLKDIV1: USBDIV=0,USBFRAC=0,USBSRC=1 */
  SIM_CLKDIV1 = (uint8_t)((SIM_CLKDIV1 & (uint8_t)~(uint8_t)0xF2U) | (uint8_t)0x01U); /* Clock source=Divided Fll0 clock Div=1 Mult=1 */
  /* Enable module clock */
  /* SIM_SCGC6: USBOTG=1 */
  SIM_SCGC6 |= (uint8_t)0x80U;                            
  /* Reset module */
  /* USB0_USBTRC0: USBRESET=1 */
  USB0_USBTRC0 = (uint8_t)0x80U;       /* Reset module */
  while (USB_PDD_GetModuleResetPendingFlag(USB0_BASE_PTR)) {
    ; /* Wait for reset done */
  }
  /* SIM_SOPT3: RWE=1 */
  SIM_SOPT3 |= (uint8_t)0x01U;         /* Enable write to the USB volatage regulator enable bit */
  /* SIM_SOPT1: REGE=1 */
  SIM_SOPT1 |= (uint8_t)0x80U;         /* Enable USB voltage regulator */
  /* SIM_SOPT7: USBBE=1 */
  SIM_SOPT7 |= (uint8_t)0x80U;         /* Enable USB Byte-swap */
  /* USB0_USBTRC0: ??=1 */
  USB0_USBTRC0 = (uint8_t)0x40U;       /* Enable internal pull-up */
  /* USB0_USBCTRL: SUSP=1,PDE=0 */
  USB0_USBCTRL = (uint8_t)0x80U;       /* Enable weak pull-downs and suspend transceiver */
  /* USB0_OTGCTL: DPHIGH=0,??=0,DPLOW=1,DMLOW=1,??=0,OTGEN=1,??=0,??=0 */
  USB0_OTGCTL = (uint8_t)0x34U;                          
  /* Set BDT address */
...
void USB_LDD_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  /* SIM_SCGC6: USBOTG=0 */
  SIM_SCGC6 &= (uint8_t)~(uint8_t)0x80U;                           
  /* {Default RTOS Adapter} Critical section end, general PE function is used */
  ExitCritical();
...
LDD_TError USB_LDD_DeviceInitEndpoint(LDD_TDeviceData *DeviceDataPtr, LDD_USB_TEpDescriptor *EpDescriptorPtr, uint8_t Flags)
{
  ...
  if (EpDescriptorPtr->bDescriptorType == LDD_USB_DT_DEVICE) {
    ...
    EpMaxPacketSize = USB_LDD_ByteRev16(EpDescriptorPtr->wMaxPacketSize);
  }
...
void USB_LDD_DeviceReadSetupData(LDD_TDeviceData *DeviceDataPtr, uint8_t EpNum, LDD_USB_TSDP *SDPPtr)
{
  ...
  *SDPPtr = *(LDD_USB_TSDP*)USB_LDD_ByteRev32(BDPtr->BufferPtr);
  USB_PDD_ClearTxSuspendFlag(USB0_BASE_PTR);               /* Restore packet transmission */
  TxEpDataPtr->TT.Ctrl.ReqDataSize = USB_LDD_ByteRev16(SDPPtr->wLength); /* Remember requested data size */
  RxEpDataPtr->TT.Ctrl.ReqDataSize = USB_LDD_ByteRev16(SDPPtr->wLength); /* Remember requested data size */
  USB_LDD_UsbUnlock(DevDataPtr);
}
...
static void USB_LDD_DeviceIsr(USB_LDD_TDeviceData *DevDataPtr, uint32_t InSignalsState)
{
  ...
  if (DevDataPtr->DeviceState >= LDD_USB_DEVICE_VBUS_OFF) { /* Device enabled and VBUS = ON? */
    ...
    if ((IntStatus & USB_PDD_TOK_DNE_INT) != 0x00U) {
      ...
      BDCtrlReg = USB_LDD_ByteRev32(BDPtr->CtrlReg);
...
PE_ISR(USB_LDD_USB_Interrupt)
{
  ...
  USB_LDD_TDeviceDataPtr DevDataPtr = Vusb0__DEFAULT_RTOS_ISRPARAM;
